...
class MoveDumanRight(Node):
    def __init__(self):
        super().__init__("move_duman_right")
    ...
    def goal_callback(self, goal_request: DumanGoal.Goal):
        ...
        if goal_request.goal_type == 0:
            ...
            if not duman_right.joint_goal_valid(self.joint_goal):
            ...
            self.right_arm_moveit.set_position_goal(position=position, frame_id=duman_right.base_link_name(), target_link=duman_right.end_effector_name())
            self.right_arm_moveit.set_orientation_goal(quat_xyzw=quat, frame_id=duman_right.base_link_name(), target_link=duman_right.end_effector_name())

            self.plan_pose_goal = self.right_arm_moveit.plan()
    ...
    def execute_callback(self, goal_handle : ServerGoalHandle):
        ...
        if goal_handle.request.goal_type == 0:
            ...
            self.right_arm_moveit.move_to_configuration(self.joint_goal)
            ...
            goal_handle.succeed()
                ...
                # self.get_logger().info(f"EXECUTING JOINT GOAL for duman right")
