...
class PicknPlace(Node):
    def __init__(self):
        ...
        self.object_poses = objects_left
        self.object_height = 0.05 # 5cm
        self.approach_ht = 0.1

        self.get_logger().info("duman left pick and place server")
    ...
    def object_cb(self, msg : Object):
        for obj, pose in zip(msg.obj_left, msg.obj_pose_left):
            if obj != "bowl":
                self.object_poses[obj] = [pose.x, pose.y, 0.18, 3.14, 0.0, 1.0] 

    def goal_callback(self, goal_request: PickNPlace.Goal):
        
        self.get_logger().info(f'{self.object_poses}')
