...
int main(int argc, char** argv)
{
  ...
  //planning grouop
  const std::string PLANNING_GROUP = "duman_arm";
  ...
  // planning plugin from the parameter server
  ...
  try
  {
    ...
    // you will see this if planner loaded successfully
  ...
  // initialie move group with node name and planning group
  moveit::planning_interface::MoveGroupInterface move_group(motion_planning_api_tutorial_node, PLANNING_GROUP);

  // Visualization
  namespace rvt = rviz_visual_tools;
  moveit_visual_tools::MoveItVisualTools visual_tools(motion_planning_api_tutorial_node, "base",
  ...
  // pose goal for the end effector
  visual_tools.trigger();
  ...
  // // tolerance to reach the goal
  // std::vector<double> tolerance_pose(3, 0.01);
  // std::vector<double> tolerance_angle(3, M_PI);

  // // create request as a constraint using kinematic constrains
  // moveit_msgs::msg::Constraints pose_goal =
  //     kinematic_constraints::constructGoalConstraints("gripper_base", pose, tolerance_pose, tolerance_goal);
  ...
  std::vector<double> joint_values = { -1.0, 0.7, 0.7, -1.5, 0.0 };
  ...
  res.getMessage(response);
  // Set the scene to the final state of the last plan 
  robot_state->setJointGroupPositions(joint_model_group, response.trajectory.joint_trajectory.points.back().positions);
  planning_scene->setCurrentState(*robot_state.get());

  // Plan and execute : To actually move the arm
